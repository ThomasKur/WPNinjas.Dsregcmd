...
#Update Version
$ModuelManifestTest = Test-ModuleManifest -Path "$ModulePath\WPNinjas.Dsregcmd.psd1" -ErrorAction Stop
...
Test-ModuleManifest -Path "$ModulePath\WPNinjas.Dsregcmd.psd1" -ErrorAction Stop
#endregion
...
#region Sign Scripts
    ...
    $PSFiles = Get-ChildItem -Path $env:TEMP\WPNinjas.Dsregcmd -Recurse | Where-Object {$_.Extension -eq ".ps1" -or $_.Extension -eq ".psm1"}
...
Publish-Module -Path $env:TEMP\WPNinjas.Dsregcmd -NuGetApiKey $PSGallerAPIKey -IconUri $Icon -LicenseUri $License -Verbose
